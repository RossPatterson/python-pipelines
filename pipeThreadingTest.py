...
try:
    importlib.reload(basicStages)
except:
    import basicStages

try:
    importlib.reload(pipethread)
except:
    import pipethread
...
class GlobalOptions(PipeAndStageOptions):
    ...
    BNF = (
        ...
        "STAGESEP xorc|SEParator xorc|ENDchar xorc|ESCape xorc|PROPAGATeeof]*",
        2,
    )
    name = "Global Options"
    listcmd = False
...
class PipeLineSet:
            ...
            while esc:
                if spec[-1] == esc and spec[-2] != esc:
                    if itemNo < len(specs):
                        ...
                        raise Error(
                            "Escape charater found at end of spec %s"
                        ).with_traceback(spec)
                else:
                    break
    ...
    def __init__(self, pipeSpec=None, **kwargs):  # PipeLineSet
        ...
        aliasedStages = [
            i[1]
            for i in list(self.stageModule.__dict__.items())
            if hasattr(i[1], "alias")
        ]
        ...
        while True:
            ...
            if not stageCls:  # look for stageName0 or stageName1 (e.g. console0)
                ...
                if stageCls:  # look for stageName0 or stageName1 (e.g. console0)
                    ...
                    if stageName[0] == "N" and issubclass(
                        stageCls, stage2.InvertibleStage
                    ):
