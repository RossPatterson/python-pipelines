...
def test():
    class Test1(SpecificationParser):
        ...
        BNF = (
            version,
            "[STReam stream|SEParator xorc|MSGlevel snumber|FOO snumber|LISTCMD|LISTERR]*",
            2,
        )
...
class SpecificationParser:
    ...
    def parseSpec(self, specs=None):
        ...
        subClass = (
            self.__class__
        )  # the class that is subclassing SpecificationParser e.g. Count
    ...
    def retrieveOrCreatePC(self, subClass):
        ...
        if len(self.BNF) == 3:  # only allowable length for now
            ...
            if (
                version is None
                or not parserCollection
                or parserCollection.version != version
            ):
                ...
                if cat in "uln":  # alphanumeric - start of token
                    ...
                    if inGroup:
                        if ch == "[":  # recursive call
                            ...
                            if self.name:
                                raise PipeExceptions.SyntaxError(
                            ...
                            if (
                                candidateIndex < len(self.BNF)
                                and self.category[candidateIndex] == "r"
                            ):
                                self.parser.repeat = self.BNF[
                                    candidateIndex
                                ]  # * 0 or more, + 1 or more, ? 0 or 1
                                if self.parser.default and self.parser.repeat == "+":
                                    raise PipeExceptions.SyntaxError(
                                        "default value conflicts with + repeat count"
                                    )
                                candidateIndex += 1
                                if (
                                    candidateIndex < len(self.BNF)
                                    and self.BNF[candidateIndex] not in " |]"
                                ):
                                    startOfToken = candidateIndex
                                    candidateIndex = self.category.find(
                                        "b", candidateIndex
                                    )
                                    if candidateIndex >= 0:
                                        self.index = candidateIndex
                                        self.parser.repeatSep = self.BNF[
                                            startOfToken:candidateIndex
                                        ]
                                    else:
                        ...
                        #              return
                            ...
                            if self.recursionDepth > 1:
                                ...
                                raise PipeExceptions.SyntaxError(
                                    "unexpected character '%s' outside group" % ch
                                )
                        else:
                            raise PipeExceptions.SyntaxError(
                                "unexpected character '%s' outside group" % ch
    ...
    def processToken(self, value=None):
        ...
        if (
            initialCharCategory == "u"
        ):  # keyWord starts with cap(s); may end with lower case
            ...
            if parser:
                ...
                raise PipeExceptions.SyntaxError(
                    "Unexpected syntactic variable %s" % (self.token,)
                )
        elif (
            initialCharCategory == "n"
        ):  # leading digit - (if not default) see apldecode et.al.
...
class Token:
    ...
    def __init__(self, sp):  # specParser
        ...
        ##      parser.expression = self.expression
            ...
            raise PipeExceptions.SyntaxError(
                "unexpected character following '%s'" % token
            )
...
class ParserCollection(list):
    ...
    def parse(self, specs, mode):
        ...
        for parser in self:
            if specs:
                values, specs = parser.parse(
                    specs, mode
                )  # seq of values of consumed tokens, leftover specs
            else:
                if parser.required:
                    raise PipeExceptions.SpecError(
                        "Missing value for %s" % (self.syntaxSpec,)
                    )
...
class SyntacticVariable(object, SpecificationParser):
    ...
    def getSvDict(self):
        ...
        if os.path.exists(fn):
            svDict = shelve.open(
                "sv.db", flag="r"
            )  # {classname(lower): instance of sv class, ...}
        ...
        if token.lower() == "string":
            ...
            if c in "hHxX":
                ...
                if len(token) % 8 == 1:
                    try:
                        value = "".join(
                            chr(int(token[b : b + 8], 2))
                            for b in range(0, len(token), 8)
                        )
                    except:
...
class StreamID(TokenSyntacticVariable):
    ...
    def subParse(self, spec):
        if spec.isdigit():
            raise PipeExceptions.SpecError(
                "streamid '%s' must not be a number" % (spec,)
            )
        return spec
