...
class Count(stage2.Stage):
  ...
  which handles what it can then ends."""
    ...
    def eof(self):
        ...
        self.output(
            " ".join(totals), streamNo=self.secondary
        )  # put total(s) in outstream
...
class Join(stage2.Stage):
    ...
    def run(self, inStream):
        if self.count > self.n:
            ...
            if self.maxlength:
                candidate = candidate[: self.maxlength]
...
class Locate(stage2.Stage, stage2.InvertibleStage):
    ...
    BNF = (
        1,
        "[ANYcase]? [MIXED|ONEs|ZEROs]? [inputranges]? [ANYof]? delimitedstring",
        1,
    )
    ...
    def initialize(self):
        ...
        if ops[0].isalpha():  # check for Left or Right
            ...
            if "LEFT".startswith(self.side) or "RIGHT".startswith(self.side):
                ...
                self.error(
                    "First operand %s should be Left or Right or integer." % self.side
                )
