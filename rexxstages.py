...
class RexxStage(stage2.Stage):
  ...
  """
    ...
    # subcommands
    def readto(self, rest):
        self.read(rest, "read")

    def peekto(self, rest):
        self.read(rest, "peek")

    def output(self, rest):
        print("output", self.rexxVars[rest])
    ...
    def callpipe(self, rest):
        print("callpipe", rest)

    def addpipe(self, rest):
        print("addpipe", rest)

    def addstream(self, rest):
        print("addstream")
        ...
        with self.cond.cond:
            if self.cond.waiting:  # read or peek pending
                ...
                if not self.consume:  # peek was pending
                    self.cond.wait(
                        thread, "run for read after releasing"
                    )  # for subsequent read
                else:
                    thread.log("run ends")
            else:
